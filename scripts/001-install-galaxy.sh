...
if [ ! -d ${gal_dir} ]; then
  runuser -l ${sge_user} -c 'git clone https://github.com/galaxyproject/galaxy.git /shared/Galaxy/galaxy-app'
fi
...
if -d ${CYCLECLOUD_SPEC_PATH}; then
  ...
  runuser -l ${sge_user} -c 'GALAXY_LOG=/shared/Galaxy/galaxy-app/galaxy.log sh /shared/Galaxy/galaxy-app/run.sh --daemon'
fi
